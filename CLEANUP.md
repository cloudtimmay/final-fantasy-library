...
- [x] **Bolk 7 — itinerary «Legg til stopp»-CSS-fiks.** Fullført og committet på `cleanup`-grena. Grundig testet og bekreftet. Se punkt 2 under **Funn under testing**.
...
- [x] **Bolk 9 — `buildDoc()`-uttrekk for de fire create-endepunktene.** Fullført og committet på `cleanup`-grena. Ren refaktorering, ingen atferdsendring — verifisert ved å opprette ett album, ett spill, en bok og en merch-vare i Studio (format `cd`, `publisher`, `author`+`publisher`+`isbn`, `category`, henholdsvis). Se punkt 2.8 nedenfor.
- [ ] Bolk 10 og videre — se **Anbefalt rekkefølge** nederst for foreslått neste steg.
...
2. ~~**Itinerary-siden: «Legg til stopp» gjør ingenting ved klikk, ingen konsoll- eller serverfeil.**~~ — **Gjennomført (bolk 7).** I `astro/src/pages/itinerary.astro` sitt add-stop-flow (`.add-stop-btn` åpner `.add-form`, `.af-save` skal kalle `addShopStop()`/`addCustomStop()`) skjedde det ingenting synlig, verken ved valg av butikk fra lista eller ved fritekst-alternativet (`.af-custom`, felt for egne stopp som frokost/reise/severdigheter) — begge veier var brutt. Fritekst-alternativet er en ønsket funksjon og ble **beholdt**.

   **Rotårsak:** `attachEvents()`-kobling, klassenavn og `di`-utregning ble gjennomgått linje for linje — ingen mismatch eller stale-listener-bug. Fant i stedet en CSS-bug: `.add-form`/`.af-custom` satte `display: flex` ubetinget, noe som (per CSS-cascade-regler — forfatter-CSS slår alltid nettleserens standard-`[hidden]`-regel) gjorde at `hidden`-attributtet aldri hadde noen visuell effekt — skjemaet sto alltid åpent, uansett hva `form.hidden`/`custom.hidden` ble satt til i JS. **Rettet** med en minimal, ren CSS-tilføyelse: `.add-form[hidden], .af-custom[hidden] { display: none; }` — ingen JS eller markup rørt. Grundig testet og bekreftet av bruker (alle seks sjekkpunktene grønne: knappen åpner/lukker skjemaet synlig, legg til fra nedtrekksliste virker, legg til fritekst virker, på flere dager, ingen regresjon).

   **Bifunn (valgfri fremtidig opprydding, ikke nødvendig nå):** `.add-form`/`.af-custom` styrer synlighet via `[hidden]`-attributtet, mens `.mc-panel` (samme fil) bruker en `.open`-klasse som veksles med `classList.toggle()`. To ulike mønstre for samme type vis/skjul-logikk i samme fil — kan harmoniseres senere om ønskelig, ikke en del av denne fiksen.
